    ...
    public class LuceneTest
    {
        ...
        public static void Show()
        {
            ...
            QueryParser parser = new QueryParser(Lucene.Net.Util.Version.LUCENE_30, "title", new PanGuAnalyzer());//解析器
            {
                ...
                {
                    Query query = parser.Parse(keyword);
                    NumericRangeFilter<int> timeFilter = NumericRangeFilter.NewIntRange("time", 20170000, 20171822, true, true);//过滤
            ...
            using (IndexWriter writer = new IndexWriter(directory, new PanGuAnalyzer(), true, IndexWriter.MaxFieldLength.LIMITED))//索引写入器
            {
                for (int i = 0; i < commodityList.Count; i++)
                {
                    for (int k = 0; k < 10; k++)
                    {
                        ...
                        doc.Add(new Field("id", commodityList[i].Id.ToString(), Field.Store.NO, Field.Index.NOT_ANALYZED));//一个字段  列名  值   是否保存值  是否分词
                        doc.Add(new Field("title", commodityList[i].Title, Field.Store.YES, Field.Index.ANALYZED));
                        doc.Add(new Field("url", commodityList[i].Url, Field.Store.NO, Field.Index.NOT_ANALYZED));
                        doc.Add(new Field("imageurl", commodityList[i].ImageUrl, Field.Store.NO, Field.Index.NOT_ANALYZED));
                        doc.Add(new Field("content", "this is lucene working,powerful tool " + i + " " + k, Field.Store.YES, Field.Index.ANALYZED));
                        doc.Add(new NumericField("price", Field.Store.YES, true).SetDoubleValue((double)(commodityList[i].Price + k)));
